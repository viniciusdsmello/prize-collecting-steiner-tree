import random
from pcstp.aco.anthill import AntHill


class Ant():
    def __init__(self, anthill: AntHill, name: str):
    ...
    def turn(self):
        if not self.has_reached_end:
            ...
            if len(self.route) == self.anthill.dimension:
                ...
                self.anthill.tracePheromone(self.route_cost(self.anthill.nodes), self.route)
    ...
    def move(self):
        ...
        for i in range(1, self.anthill.dimension + 1):
            ...
            eta = (self.anthill.weight_max - self.anthill.weight_min) / (dist - self.anthill.weight_min + 1)
            if self.testFlag:
                eta = 1
